...
#usage: python conevainter.py <rr_file_inter_true> <rr_file_predicted>
from readRR import *
import numpy as np
import os,sys
import subprocess
        ...
        for rr2_item in rr2:
            ...
            for rr1_item in rr1:
                ...
                if (select == rr1_item_ij): 
                    ...
                    #print(select)
                    #break
                if (select_rev == rr1_item_ij and select_rev!=select):
                    contact_num+=1
                    contact_list.append(select)
                    #Remark the following to switch to mirror double consideration
                    #continue
                    ...
                    #break
                #else:
                #    notfound_list.append(select)
        #print (len(rr2),len(rr1),contact_num)
        #print (list(set(contact_list)))
        #print ("Notfound:\n",list(notfound_list))
    #else do relaxation
    else:
        #return
...
#    if (top=="L/2" or top=="l/2"): val=int(round((L/2),0))
    ...
    if (top=="L/20" or top=="l/20"): val=int((L/20)+0.5)
    if (top=="L/30" or top=="l/30"): val=int((L/30)+0.5)
...
def getPrecision(rr1,rr2,fasta,top="All",relax=0):
    ...
    if (top=="L/10" or top=="l/10"): val=int(round((L/10),0))
    if (top=="L/5" or top=="l/5"): val=int(round((L/5),0))
    if (top=="L/20" or top=="l/20"): val=int(round((L/20),0))
    if (top=="L/30" or top=="l/30"): val=int(round((L/30),0))
    #if (top=="2L" or top=="2l"): val=int(2*L)
...
def getPrecisionMyWay(rr1,rr2,fasta,top="All",relax=0):
    ...
    if (top=="L/20" or top=="l/20"): val=int(L/20)
    if (top=="L/30" or top=="l/30"): val=int(L/30)
    #if (top=="2L" or top=="2l"): val=int(2*L)
...
usage_inter="python conevainter.py <rr_file_inter_true> <rr_file_predicted>" # 6BZE_AE.rr 6BZE_A.dncon2.rr 6BZE_AA.rr 

df_list=[]

contact_list=[]
print("ASDFADSFADSFADSFADSFASDF    Starts")
if (len(sys.argv)!=4): 
    print(len(sys.argv))
    sys.exit("Please enter 3 valid arguments\n"+usage_inter)
else:
    rr_file1=os.path.abspath(sys.argv[1]) #always the native file- the interchain contact file
    rr_file2=os.path.abspath(sys.argv[2]) #the prediction file or .dncon2.rr file
    fasta_file=sys.argv[3]
    print (os.path.dirname(os.path.abspath(sys.argv[0])))
    print ("Running: "+sys.argv[0]+ " for "+getName(rr_file1))
    name=getName(rr_file1)
    nm=getName(fasta_file)
    #print (rr_file1)
    #sort the dncon2.rr file
    dirnm=os.path.dirname(os.path.abspath(sys.argv[0]))+"/"
    print (dirnm)
    #print ("Name is: "+nm)
    with open ("/data/farhan/SoftwareTools/HomopolymerProject/DeepComplex/training/dncon2_model/"+"lost.txt","w") as f:
        f.write(dirnm+nm+"\n")
        f.write(nm+"\n")
        f.write(dirnm+nm+"_temp_rr.rr")
    print ("I am here!!!!!!!")
    #sys.exit()
    #os.system("python "+dirnm+"Label2cmap.py "+rr_file1+" "+dirnm+nm+"_temp_rr.rr " +fasta_file)
    subprocess.check_call("python "+dirnm+"Label2cmap.py "+rr_file1+" "+dirnm+nm+"_temp_rr.rr " +fasta_file, shell=True)
    rr_file1=dirnm+nm+"_temp_rr.rr"
    #os.system("python "+dirnm+"sortrr.py "+rr_file2+" True > "+rr_file2.replace(".rr","_sorted.rr"))
    subprocess.check_call("python "+dirnm+"sortrr.py "+rr_file2+" True > "+rr_file2.replace(".rr","_sorted.rr"),shell=True)
    ...
    fst=""
    fasta1,rr1=readRRFile(rr_file1) #multimer PDB interchain rr (True)
...
#    print (L,len(fasta3),fasta3)
    ...
    relaxation=1 #precision relaxation = 2
    top={}
...
############################################################
    ...
    for relax_prec in range(relaxation):
        ...
        top["Top-20"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"20",relax_prec),2))
        top["Top-30"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"30",relax_prec),2))
        top["Top-50"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"50",relax_prec),2))
        top["Top-L/30"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L/30",relax_prec),2))
        top["Top-L/20"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L/20",relax_prec),2))
        ...
        string3=name+"        "+top["Top-5"]+"         "+top["Top-10"]+"         "+top["Top-L/30"]+"          "+top["Top-L/20"]+"          "+top["Top-L/10"]+"          "+top["Top-L/5"]
