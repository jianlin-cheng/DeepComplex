...
"""
rr_file1="./4zuk_out/4zuk_multimer_dist_8_AB_dist.txt" #always the native file
rr_file2="./sorted_4zuk.rr" #the prediction file
rr_file_intra="./4zuk_out/4zuk_monomer_distance_next_AA.native.txt"
...
print("Initial=",len(rr1),len(rr2),len(rr_intra)) #initial
...
print("Seq_sep<6 removed =",len(rr1),len(rr2),len(rr_intra)) #after removing seq sep < 6
...
print("RR trimmed 2L =",len(rr1),len(rr2),len(rr_intra)) #trimmer to 2L

#rr2=removeIntraContacts(rr2,rr_intra)

rr2=removeIntraContactsVicinity(L,rr2,rr_intra,6)

#ff,rr_t=readRRFile("temp_rr.txt")
#print("Len=",len(rr_t))

print("Intrachain removed =",len(rr1),len(rr2),len(rr_intra)) #not intrachain

write2File(makeOutputFile(rr_file2,"_nointra.rr"), fasta2, rr2)#trimRR(rr2,2*L))
#rr2=removeIntraContacts(rr2,rr_intra)
...
#print(len(rr1),len(rr2),len(rr_intra))

#print(round(getPrecision(rr_intra,rr2,fasta2,"L/5",0),2))

relax_prec=2
print(round(getPrecisionInter(rr1,rr2,fasta2,"5",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"10",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"L/10",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"L/5",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"L/2",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"L",relax_prec),2))
print(round(getPrecisionInter(rr1,rr2,fasta2,"2L",relax_prec),2))

"""
"""
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"5",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"10",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"L/10",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"L/5",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"L/2",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"L",2),2))
print(round(getPrecisionMyWay(rr1,rr2,fasta2,"2L",2),2))
"""
...
print(round(getPrecision(rr1,rr2,fasta2,"5",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"10",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"L/10",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"L/5",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"L/2",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"L",0),2))
print(round(getPrecision(rr1,rr2,fasta2,"2L",0),2))
"""
usage_intra="python getPrecision.py <intra> <pdbfile> <rr_file>"
usage_inter="python getPrecision.py <inter> <rr_file_inter_true> <rr_file_predicted> <rr_file_intra_true>"
usage=usage_intra+"\n"+usage_inter
what=sys.argv[1] #inter or intra
#print(what)
if (what=="inter" or what=="intra"): 
    pass
else:
    sys.exit("Please specify what to get inter or intra chain precision\n"+usage)

if (what =="intra"):
    if (len(sys.argv)!=4): sys.exit("Please enter 3 valid arguments\n"+usage_intra)
    pdbfile=sys.argv[2]
    rr_file=sys.argv[3]
    os.system("perl /data/farhan/SoftwareTools/HomopolymerProject/scripts/perl_scripts/coneva.pl -pdb "+pdbfile+" -rr "+rr_file+" > temp_coneva.txt")
    print(printFromConeva())
    pass
else:
    print(len(sys.argv))
    if (len(sys.argv)!=5): sys.exit("Please enter 4 valid arguments\n"+usage_inter)
    rr_file1=sys.argv[2] #always the native file- the interchain contact file
    rr_file2=sys.argv[3] #the prediction file
    rr_file_intra=sys.argv[4] #the true intra chain contacts
    #print(rr_file1)
    #print(rr_file2)
    #print(rr_file_intra)
    #sys.exit()
    ...
    for relax_prec in range (relaxation):
        ...
        top["5"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"5",relax_prec),2))
        top["10"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"10",relax_prec),2))
        top["L/10"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L/10",relax_prec),2))
        top["L/5"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L/5",relax_prec),2))
        top["L/2"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L/2",relax_prec),2))
        top["L"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"L",relax_prec),2))
        top["2L"]=str(round(getPrecisionInter(rr1,rr2,fasta2,"2L",relax_prec),2))
        
        string3=name+"        "+top["5"]+"         "+top["10"]+"         "+top["L/10"]+"          "+top["L/10"]+"          "+top["L/2"]+"          "+top["L"]+"          "+top["2L"]
