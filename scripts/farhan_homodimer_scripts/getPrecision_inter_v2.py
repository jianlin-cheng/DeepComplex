...
if (len(sys.argv)!=4): 
    ...
    rr_file2=sys.argv[2] #the prediction file
    ...
    fasta1,rr1=readRRFile(rr_file1) #multimer PDB interchain rr
    fasta2,rr2=readRRFile(rr_file2) #prediction rr(dncon2.rr)
    fasta3,rr_intra=readRRFile(rr_file_intra)#intra-chain contacts from monomer PDB
    L=len(fasta2)
    ...
    for relax_prec in range (relaxation):
        rr2=removeIntraContactsVicinity(L,rr2,rr_intra,relax_prec) #relaxation = 3. we can change
        #write2File(makeOutputFile(rr_file2,"_filtered.rr"), fasta2, trimRR(rr2,2*L))
    
        
        write2File(makeOutputFile(rr_file2,"_nointra_relax_"+str(relax_prec)+".rr"), fasta2, rr2)#trimRR(rr2,2*L))
