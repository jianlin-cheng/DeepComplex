    ...
    if (relax==0):
        for rr2_item in rr2:
            ...
            for rr1_item in rr1:
                ...
                j1=split2[1]
                #if (i1==i2 and j1==j2): 
                if (select == i1+" "+j1):#if (select in rr1_item):
                    contact_num+=1
                    break
...
def getPrecision(rr1,rr2,fasta,top="All",relax=0):
    ...
    if (top=="5"): val=5
    if (top=="10"): val=10
    if (top=="L/10" or top=="l/10"): val=int((L/10)+0.5) #val=int(round((L/10),0))
    if (top=="L/5" or top=="l/5"): val=int((L/5)+0.5) #val=int(round((L/5),0))
    if (top=="L/2" or top=="l/2"): val=int((L/2)+0.5) #val=int(round((L/2),0))
    if (top=="L" or top=="l"): val=int(L)
    if (top=="2L" or top=="2l"): val=int(2*L)
...
#    write2File(outfile_pred,fasta2,rr2)
    ...
    #print ("L=",L,"Val=",val,"con_num=",con_num)
    #analyze("temp(1).rr",rr1)
...
rr_file2=rr_file2.replace(".rr","_filtered.rr")

#sort the dncon2.rr file
os.system("python sortrr.py "+rr_file2+" > "+rr_file2.replace("_filtered.rr","_sorted_filtered.rr")+" True")
rr_file2=rr_file2.replace("_filtered.rr","_sorted_filtered.rr")
fasta2,rr2=readRRFile(rr_file2) #read the sorted and filtered file now prediction rr
