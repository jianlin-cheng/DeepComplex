#this scripts selects the pdbs whose fasta sequences match in the .seq file

#usage:
#python select_pdb_not_fasta_mismatch.py 
...
pdb_list=[]
with open ("true_pdb_list.txt","r") as f:
    for line in f:
...
if (not os.path.exists(outfolder+"fastas_no_mismatch")): os.mkdir(outfolder+"fastas_no_mismatch")

removeBlankFiles(pdb_list)

#1. read .seq files and match the fasta sequences
#updated_pdb_list=[]
"""
for pdb in pdb_list:
...
for pdb in pdb_list:
    ...
    lnnum = str(lnnum)
    lnnum = int(lnnum.strip("b").strip("'"))-1
    if (lnnum==0):continue #move on for empty files
    ...
    with open ("seq_list.lst","r") as f:
        for line in f:
            ...
            if (contents[4]!=fasta and fasta!=""): #compare the fasta length of all .seq files 
                break
            #l1=len(contents[4])
            #l2=len(fasta)
            ...
            L=int(contents[3])
            seq_file_list=[]
            seq_file_list.append(outfolder+"fastas_no_mismatch/"+pdb_name+".fasta")
...
with open ("updated_pdb_list_new.lst","w") as f:
