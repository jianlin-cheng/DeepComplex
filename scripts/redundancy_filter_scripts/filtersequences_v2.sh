#!/bin/bash
#usage: sh filtersequences_v2.sh <fasta_file> <threshold> <outputfile>
export PATH=$PATH:/storage/htc/bdm/tools/MMseqs2_static/bin
...
mmseqs result2flat sequenceDB sequenceDB DB_clu_rep $3_$2.txt --use-fasta-header
if [ ! $?==0 ]; then
    echo "######### 5th: Error $?"
    exit
fi
echo "Filtered fasta file called $3_$2.txt created!"
