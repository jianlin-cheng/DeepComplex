...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 13 16:36:49 2020
...
checkpoint_file=os.path.abspath(sys.argv[4]) #this is the completion.txt file that stores previous training checkpoint.

###################################################################################
################### Current Algorithm used in training ############################
# Read the checkpoint file to find the epoch and the length category (range) trained so far
# If checkpoint file does not exist, or is empty, then previous_epoch = 0 so restart_epoch=previous_epoch+1
# And length training range is [30,100], [101,200], [201,300], ...
# Establish restart_epoch and training range as before
# Load the last category range weight as current weight
# Load model and train for next epoch. 
# When training is done save weights and update the checkpoint file.
# Move to next category and repeat.
# if all categories are completed, save weight for this cycle. Start training from all over again.
###################################################################################
...
# Read the checkpoint file here
##################
freq_distr={} #All
train_distr={} #train
val_distr={} #Validation
test_distr={} #test
...
with open (project_root+"scripts/data_distr_100.txt","r") as f:
    for line in f:
        ...
        if low_limit<30: low_limit=29
        up_limit=int(line.strip().split()[1].strip("(").strip("[").strip(")").strip("]").strip(","))
        freq_distr[str(low_limit+1)+","+str(up_limit)]=freq
    
with open (project_root+"scripts/train_distr_100.txt","r") as f:
    for line in f:
        freq=int(line.strip().split()[-1])
        low_limit=int(line.strip().split()[0].strip("(").strip("[").strip(")").strip("]").strip(","))
        if low_limit<30: low_limit=29
        up_limit=int(line.strip().split()[1].strip("(").strip("[").strip(")").strip("]").strip(","))
        train_distr[str(low_limit+1)+","+str(up_limit)]=freq

with open (project_root+"scripts/val_distr_100.txt","r") as f:
    for line in f:
        freq=int(line.strip().split()[-1])
        low_limit=int(line.strip().split()[0].strip("(").strip("[").strip(")").strip("]").strip(","))
        if low_limit<30: low_limit=29
        up_limit=int(line.strip().split()[1].strip("(").strip("[").strip(")").strip("]").strip(","))
        val_distr[str(low_limit+1)+","+str(up_limit)]=freq

with open (project_root+"scripts/test_distr_100.txt","r") as f:
    for line in f:
        freq=int(line.strip().split()[-1])
        low_limit=int(line.strip().split()[0].strip("(").strip("[").strip(")").strip("]").strip(","))
        if low_limit<30: low_limit=29
        up_limit=int(line.strip().split()[1].strip("(").strip("[").strip(")").strip("]").strip(","))
        test_distr[str(low_limit+1)+","+str(up_limit)]=freq


cycle_flag=False #To check if one complete training cycle has finished

current_weight_file=""
checkpoint_epoch=-1
restart_epoch=1
# Read the checkpoint file and read the last completed category information
if os.path.exists(project_root+"scripts/checkpoint.txt"):
    with open (project_root+"scripts/checkpoint.txt","r") as f:
        for line in f:
            # Epoch    Category:Low_limit      Up_limit
            checkpoint_epoch=int(line.strip().split()[0])
            low_limit=int(line.strip().split()[1])
            up_limit=int(line.strip().split()[2])
            current_weight_file="weights_"+str(low_limit)+"_"+str(up_limit)+".hdf5"
    # Set the next epoch, and category
    if up_limit>=maxLengthinDataset:
        restart_epoch=checkpoint_epoch+1 # Next epoch
        low_limit=30 # Restart from first range
        up_limit=100
        cycle_flag=True
        #pass
    else:
        restart_epoch=checkpoint_epoch # Continue with current epoch
        low_limit=up_limit+1 # Next range
        up_limit=up_limit+100
        while freq_distr[str(low_limit)+","+str(up_limit)]==0: # No data available in this range, so move to next range
            low_limit=up_limit+1 # Next range
            up_limit=up_limit+100
  
# if checkpoint.txt file does not exist then it means it is the start
else:
    #os.system("rm weights*.hdf5") #Remove existing weights
    #os.system("rm cycle-*.hdf5")
    checkpoint_epoch=0
    restart_epoch=1
    low_limit=30
    up_limit=100
next_weight_file="weights_"+str(low_limit)+"_"+str(up_limit)+".hdf5"
print (freq_distr)
freq=freq_distr[str(low_limit)+","+str(up_limit)]

working_range=str(low_limit)+"_"+str(up_limit)

print ('Loading all Training data into memory..')
print ("Minimum length: ",low_limit)
print ("Minimum length: ",up_limit)
...
tr_freq=1 #train_distr[working_range]
val_freq=1 #val_distr[working_range]
test_freq=1 #test_distr[working_range]

#################
#put the code to manage zero validation and test counts. Set the same as training.
#################

LTR1 = subset_pdb_dict(train_l, low_limit, up_limit, tr_freq, 'random')
LTR.append(LTR1)
print (len(LTR1))
#print("Typr of LTR1=",type(LTR1))
print ('Loading sets input feature matrix X..')
XTR1 = get_x_from_this_list(LTR1, pathX, up_limit)
XTR.append(XTR1)
#print("Typr of XTR1=",type(XTR1))
print ('Loading label matrix Y ..')
YTR1 = get_y_from_this_list(LTR1, pathY, 0, up_limit, dist_string)
YTR.append(YTR1)
...
LTV1 = subset_pdb_dict(val_l, low_limit, up_limit, val_freq, 'ordered')
LTE1 = subset_pdb_dict(test_l, low_limit, up_limit, test_freq, 'ordered')
if len(LTV1)==0 and len(LTE1)==0: #if validation and test data are missing
    LTV1=LTR1
    LTE1=LTR1
elif len(LTV1)==0:
    LTV1=LTE1
...
XTV1 = get_x_from_this_list(LTV1, pathX, up_limit)
XTR.append(XTV1)
print ('Loading label matrix Y ..')
#YTV1 = get_y_from_this_list(LTV1, pathY, 24, len(val_l), dist_string)
YTV1 = get_y_from_this_list(LTV1, pathY, 24, up_limit, dist_string)
YTR.append(YTV1)
sys.stdout.flush()
...
XTE1 = get_x_from_this_list(LTE1, pathX, up_limit)
XTR.append(XTE1)
print ('Loading label matrix Y ..')
YTE1 = get_y_from_this_list(LTE1, pathY, 24, up_limit, dist_string)
YTR.append(YTE1)
...
for cyc in range (rerun_epoch-1, rerun_epoch):
#for cyc in range (rerun_epoch, 21):
    ...
    print (list(LTR.keys()))
    # Save weights as the category: weights_low_up.hdf5
    if os.path.exists(project_root+"scripts/"+current_weight_file): print ("Loading already saved weights file: "+project_root+"scripts/"+current_weight_file)
    train_on_this_X_Y(model_arch, train_param, X, Y, project_root+"scripts/"+current_weight_file ,project_root+"scripts/"+next_weight_file)
    print ('')
    # if cycle is complete save final weights.
    if cycle_flag:
        print ('Save the weights & model for this cycle..')
        shutil.copy2(project_root+"scripts/"+next_weight_file, project_root+"scripts/"+'cycle-' + str(cyc) + '.hdf5')
    #group = next_group(group)
    print ('')
    cyc_id = str(cyc)
    cyc_id = cyc_id.rjust(5, '0')
    print ("Evaluating on training data:")
    (acc_T5, acc_T10, acc_T20, acc_T30, acc_T50, acc_l30, acc_l20, acc_l10, acc_l5)=print_detailed_accuracy_on_this_data(cyc_id + ' SAMPLE-TRAIN', next_weight_file,LTR,XTR,YTR,pathX,pathY,epochs=cyc)
    with open ("training_acc_"+str(low_limit)+"_"+str(up_limit)+".txt","a+") as f:
        epoch = cyc
        if (epoch == 0):
            f.write("Epoch\tSample_Size\tPrec-T5\tPrec-T10\tPrec-T20\tPrec-T30\tPrec-T50\tL/30\tL/20\tL/10\tL/5\n")
        f.write(str(epoch)+"\t"+str(len(LTR1))+"\t"+str(round(acc_T5,4))+"\t"+str(round(acc_T10,4))+"\t"+str(round(acc_T20,4))+"\t"+str(round(acc_T30,4))+"\t"+str(round(acc_T50,4))+"\t"+str(round(acc_l30,4))+"\t"+str(round(acc_l20,4))+"\t"+str(round(acc_l10,4))+"\t"+str(round(acc_l5,4))+"\n")
        print("Train Accuracies: \n"+str(epoch)+"\t"+str(len(LTR1))+"\t"+str(round(acc_T5,4))+"\t"+str(round(acc_T10,4))+"\t"+str(round(acc_T20,4))+"\t"+str(round(acc_T30,4))+"\t"+str(round(acc_T50,4))+"\t"+str(round(acc_l30,4))+"\t"+str(round(acc_l20,4))+"\t"+str(round(acc_l10,4))+"\t"+str(round(acc_l5,4))+"\n")
        
    print ('Evaluate on Validation data..')
    (acc_T5, acc_T10, acc_T20, acc_T30, acc_T50, acc_l30, acc_l20, acc_l10, acc_l5)=print_detailed_accuracy_on_this_data(cyc_id + ' VAL-TRAIN', next_weight_file,LTV,XTV,YTV,pathX,pathY,epochs=cyc)
    with open ("validation_acc_"+str(low_limit)+"_"+str(up_limit)+".txt","a+") as f:
        epoch = cyc
        if (epoch == 0):
            f.write("Epoch\tSample_Size\tPrec-T5\tPrec-T10\tPrec-T20\tPrec-T30\tPrec-T50\tL/30\tL/20\tL/10\tL/5\n")
        f.write(str(epoch)+"\t"+str(len(LTR1))+"\t"+str(round(acc_T5,4))+"\t"+str(round(acc_T10,4))+"\t"+str(round(acc_T20,4))+"\t"+str(round(acc_T30,4))+"\t"+str(round(acc_T50,4))+"\t"+str(round(acc_l30,4))+"\t"+str(round(acc_l20,4))+"\t"+str(round(acc_l10,4))+"\t"+str(round(acc_l5,4))+"\n")
    ...
    (acc_T5, acc_T10, acc_T20, acc_T30, acc_T50, acc_l30, acc_l20, acc_l10, acc_l5)=print_detailed_accuracy_on_this_data(cyc_id + ' ALL-TEST    ', next_weight_file,LTE,XTE,YTE,pathX,pathY,epochs=cyc)
    with open ("testing_acc_"+str(low_limit)+"_"+str(up_limit)+".txt","a+") as f:
        epoch = cyc
        if (epoch == 0):
            f.write("Epoch\tSample_Size\tPrec-T5\tPrec-T10\tPrec-T20\tPrec-T30\tPrec-T50\tL/30\tL/20\tL/10\tL/5\n")
        f.write(str(epoch)+"\t"+str(len(LTR1))+"\t"+str(round(acc_T5,4))+"\t"+str(round(acc_T10,4))+"\t"+str(round(acc_T20,4))+"\t"+str(round(acc_T30,4))+"\t"+str(round(acc_T50,4))+"\t"+str(round(acc_l30,4))+"\t"+str(round(acc_l20,4))+"\t"+str(round(acc_l10,4))+"\t"+str(round(acc_l5,4))+"\n")
...
with open (project_root+"/scripts/checkpoint.txt","w") as f:
    f.write(str(rerun_epoch)+" "+str(low_limit)+" "+str(up_limit))
