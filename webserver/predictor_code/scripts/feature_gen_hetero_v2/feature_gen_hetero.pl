...
# Modified by Tianqi, 10-03-2017, add function generate_pssm_pseudo
...
####################################################################################################
use constant{
	SCRATCH      => '/storage/htc/bdm/tools/SCRATCH-1D_1.1/bin/run_SCRATCH-1D_predictors.sh',
	FORMATDBPATH      => '/storage/htc/bdm/tools/SCRATCH-1D_1.1/pkg/blast-2.2.26/bin/formatdb', # added by Tianqi on 10/03/2017
	BLASTPATH    => '/storage/htc/bdm/tools/ncbi-blast-2.2.25+/bin',
	BLASTNRDB    => '/storage/htc/bdm/tools/databases/nr90_2016',#needs to be updated to nr90_2020 (needs to be created)#'/storage/htc/bdm/tools/databases/nr90-2012',
	#BLASTNRDB    =>  '/storage/htc/bdm/tools/nr_database_updated', # Updated by Tianqi on 10/30/2017
	PSIPRED      => '/storage/htc/bdm/tools/MetaPSICOV/runpsipredandsolv',
	ALNSTAT      => '/storage/htc/bdm/tools/MetaPSICOV/bin/alnstats',
	FEAT2RR      => abs_path(dirname($0)).'/predict-rr-from-features.sh',
	ALNSCRIPT    => abs_path(dirname($0)).'/generate-alignments_hetero.pl',
	RRPREDSCRIPT => abs_path(dirname($0)).'/run-ccmpred-freecontact-psicov.pl',
	GENFEAT      => abs_path(dirname($0)).'/generate-dncon2-features.pl',#'/generate-dncon2-features_inter.pl',
	HOURLIMIT    => 1
};
...
system_cmd("echo ".$s_seq." >> $outdir/$id.fasta");
print ("Seq_fasta:\n".seq_fasta($fasta)."\n");
print ("S_seq:\n".$s_seq."\n");
...
system_cmd("cp ../$fasta ./");
if (-s "$id.ss"){
	print "Looks like .ss file is already here.. skipping..\n";
}
else{
...
############# No need for the following. Remove them when appropriate ##########################################################
...
sub generate_pssm_pseudo{
	...
	system_cmd(FORMATDBPATH. "  -i ".$FASTA);
	system_cmd(BLASTPATH. "/psiblast -query ".$FASTA." -evalue .001 -inclusion_ethresh .002 -db $FASTA -num_iterations 3 -outfmt 0 -out $REPORT -seg yes -out_ascii_pssm $PSSM");
}
